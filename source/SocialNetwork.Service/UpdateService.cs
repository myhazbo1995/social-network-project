...
namespace SocialNetwork.Service
{
    ...
    public class UpdateService : IUpdateService
    {
        ...
        //public UpdateService(IUpdateRepository updateRepository, IPostRepository postRepository, IUnitOfWork unitOfWork, IFollowUserRepository followUserRepository)
        //{
        //    this.updateRepository = updateRepository;
        ...
        public Update GetLastUpdatePost(string userid)
        {
            var updates = updateRepository.GetMany(g => g.Post.UserId == userid).Last();
            return updates;
        }
        ...
        public IEnumerable<Update> GetUpdatesForaUserPost(string userid)
        {

            var updates = updateRepository.GetMany(u => (u.Post.UserId == userid)).OrderByDescending(u => u.UpdateDate).ToList(); 
            return updates;
        }
        ...
        public IEnumerable<Update> GetTop20UpdatesOfFollowingPost(string userid)
        {
            var updates = from u in updateRepository.GetAll() where (from f in followUserRepository.GetMany(fol => fol.FromUserId == userid) select f.ToUserId).ToList().Contains(u.Post.UserId) select u;
            return updates;
        }
        ...
        public IEnumerable<Update> GetTop20UpdatesPost(string userid)
        {

            var updates = updateRepository.GetMany(u => u.Post.UserId == userid).OrderByDescending(u => u.UpdateDate).Take(20).ToList(); ;
            return updates;
        }
        ...
        public IEnumerable<Update> GetUpdatesByPost(int postid)
        {
            var updates = updateRepository.GetMany(u => u.PostId == postid).OrderByDescending(u => u.UpdateId).ToList();
            return updates;
        }
